...
export const CommonPage:React.FC<CommonPagePropsType> = ({children, hasHeaderContent}) => {
            ...
            <Layout className="centerContentWrapper">
                <Header className={`${isCollapseSider ? "collapsed-style-header" : ""} ${hasHeaderContent ? "header-with-content" : "header-without-content"}`}>
                    <Breadcrumb>
                        ...
                        {(location.pathname === AppRoutes.FEEDBACK) && <Breadcrumb.Item>Отзывы пользователей</Breadcrumb.Item>}
                    </Breadcrumb>
                    {children.header}
                </Header>
                <Content >
